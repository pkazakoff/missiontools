...
from missiontools.coverage import (
    sample_aoi,
    sample_region,
    sample_shapefile,
    sample_geography,
...
class TestCoverageFraction:
    def test_output_keys(self):
        lat, lon = _sample_uk()
        result = coverage_fraction(lat, lon, _ISS, _T_START, _T_END, max_step=_STEP)
    ...
    def test_empty_window_returns_empty(self):
        lat, lon = _sample_uk()
        result = coverage_fraction(lat, lon, _ISS, _T_START, _T_START, max_step=_STEP)
    ...
    def test_batching_consistent(self):
        ...
        kwargs = dict(
            keplerian_params=_ISS, t_start=_T_START, t_end=_T_END, max_step=_STEP
        )
    ...
    def test_j2_propagator_runs(self):
        ...
        result = coverage_fraction(
            lat, lon, _ISS, _T_START, _T_END, propagator_type="j2", max_step=_STEP
        )
...
class TestRevisitTime:
    def test_output_keys(self):
        lat, lon = _sample_uk()
        result = revisit_time(lat, lon, _ISS, _T_START, _T_END, max_step=_STEP)
    ...
    def test_empty_window_returns_nan(self):
        ...
        result = revisit_time(lat, lon, _ISS, _T_START, _T_START, max_step=_STEP)
...
class TestSampleRegion:
    ...
    def test_denser_density_gives_more_points(self):
        ...
        lat_a, _ = sample_region(point_density=2e5)
        lat_b, _ = sample_region(point_density=1e5)
    ...
    def test_lon_band_respects_bounds(self):
        ...
        _, lon = sample_region(lon_min=lo, lon_max=hi, point_density=1e5)
    ...
    def test_antimeridian_no_points_in_gap(self):
        ...
        _, lon = sample_region(lon_min=lo, lon_max=hi, point_density=2e5)
    ...
    def test_antimeridian_has_points_on_both_sides(self):
        ...
        _, lon = sample_region(lon_min=lo, lon_max=hi, point_density=5e5)
    ...
    def test_all_none_is_global(self):
        ...
        lat, lon = sample_region(point_density=5e6)
        assert lat.min() < np.radians(-60.0)
        assert lat.max() > np.radians(60.0)
    ...
    def test_nonpositive_density_negative_raises(self):
        with pytest.raises(ValueError, match="point_density"):
            sample_region(point_density=-1e4)
...
class TestCoverageFractionFov:
    def test_wide_nadir_fov_matches_no_fov(self):
        ...
        lat, lon = _sample_uk(20)
        kw = dict(keplerian_params=_ISS, t_start=_T_START, t_end=_T_END, max_step=_STEP)
        r_base = coverage_fraction(lat, lon, **kw)
        r_fov = coverage_fraction(
            lat,
            lon,
            **kw,
    ...
    def test_pointing_unnormalised_matches_normalised(self):
        ...
        r_norm = coverage_fraction(lat, lon, **kw, fov_pointing_lvlh=_NADIR_LVLH)
        r_scaled = coverage_fraction(
            lat, lon, **kw, fov_pointing_lvlh=_NADIR_LVLH * 5.0
        )
...
def _kw_pw(**extra):
    ...
    return dict(
        keplerian_params=_ISS, t_start=_T_START, t_end=_T_END, max_step=_STEP, **extra
    )
...
class TestPointwiseCoverage:
    ...
    def test_with_fov(self):
        ...
        fov = pointwise_coverage(
            lat, lon, **kw, fov_pointing_lvlh=nadir, fov_half_angle=np.radians(30.0)
        )
...
class TestAccessPointwise:
    ...
    def test_empty_window(self):
        lat, lon = _sample_uk(5)
        kw = dict(
            keplerian_params=_ISS, t_start=_T_START, t_end=_T_START, max_step=_STEP
        )
...
class TestSampleShapefile:
    def test_returns_two_arrays(self, tmp_path):
        ...
        lat, lon = sample_shapefile(path, point_density=5e6)
        assert isinstance(lat, np.ndarray)
        assert isinstance(lon, np.ndarray)
        assert lat.shape == lon.shape
        assert len(lat) > 0
    ...
    def test_denser_density_gives_more_points(self, tmp_path):
        path = _make_shapefile(tmp_path, _BOX_10E)
        _, lon_coarse = sample_shapefile(path, point_density=2e5)
        _, lon_dense = sample_shapefile(path, point_density=5e4)
    ...
    def test_feature_index_single(self, tmp_path):
        ...
        lat_f0, _ = sample_shapefile(path, feature_index=0, point_density=1e4)
        assert len(lat_f0) > 0
    ...
    def test_polygon_with_hole(self, tmp_path):
        ...
        lat, lon = sample_shapefile(path, point_density=1e5)
    ...
    def test_nonexistent_file_raises(self, tmp_path):
        with pytest.raises(
            Exception
        ):  # pyshp raises ShapefileException (OSError subclass)
